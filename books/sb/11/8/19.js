var text = {
    "info": {
        "id": "sb/11/8/19",
        "prevId": "sb/11/8/18",
        "nextId": "sb/11/8/20"
    },
    "verses": [
        {
            "roman": "jihvayäti-pramäthinyä\njano rasa-vimohitaù\nmåtyum åcchaty asad-buddhir\nménas tu baòiçair yathä\n"
        }
    ],
    "synonyms": "jihvayä—by the tongue; ati-pramäthinyä—which is extremely disturbing; janaù—a person; rasa-vimohitaù—bewildered by attraction to taste; måtyum—death; åcchati—achieves; asat—useless; buddhiù—whose intelligence; ménaù—the fish; tu—indeed; baòiçaiù—by the hooks; yathä—just as.",
    "translation": "Just as a fish, incited by the desire to enjoy his tongue, is fatally trapped on the fisherman's hook, similarly, a foolish person is bewildered by the extremely disturbing urges of the tongue and thus is ruined.",
    "purport": [
        {
            "type": "normal",
            "content": "The fisherman places meaty bait on a sharp hook and easily attracts the unintelligent fish, who is greedy to enjoy its tongue. Similarly, people are mad after gratifying their tongues and lose all discrimination in their eating habits. For momentary gratification they construct huge slaughterhouses and kill millions of innocent creatures, and by inflicting such atrocious suffering they prepare a ghastly future for themselves. But even if one eats only the foods authorized in the Vedas, there is still danger. One may eat too sumptuously and then the artificially stuffed belly will create pressure on the sexual organs. Thus one will fall down into the lower modes of nature and commit sinful activities that lead to the death of one's spiritual life. From the fish one should carefully learn the real dangers involved in gratifying the tongue."
        }
    ]
}

